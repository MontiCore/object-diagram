...
public class OD2CDObjectVisitor implements ODBasisVisitor2 {
  ...
  public void handleLinks(ASTODLink odLink) {
    List<String> leftSides =
        odLink.getODLinkLeftSide().getReferenceNamesList().stream().map(ASTODName::getName)
            .collect(Collectors.toList());
    
    List<String> rightSides =
        odLink.getODLinkRightSide().getReferenceNamesList().stream().map(ASTODName::getName)
            .collect(Collectors.toList());
    
    String roleName =
        odLink.getODLinkRightSide().isPresentRole() ? odLink.getODLinkRightSide().getRole() : "";
    ...
    if (odLink.getODLinkDirection() instanceof ASTODBiDir ||
        odLink.getODLinkDirection() instanceof ASTODUnspecifiedDir) {
      for (String left : leftSides) {
        for (String right : rightSides) {
          this.linkAttributeList.add(left + ".add" + ((roleName.isEmpty())
              ? capFirst(right)
